...
const renderTemplate = async (template, req, res) => {
  ...
  const output = await carboneRenderer.render(
    template.path,
    data,
    options,
    formatters
  ...
  if (output.success) {
    ...
    if (truthy("cacheReport", options)) {
      const rendered = await fileCache.write(
        output.report,
        output.reportName,
        "binary"
      );
...
const getFromCache = async (hash, hashHeaderName, download, remove, res) => {
  ...
  if (remove) {
    ...
    if (!removed.success) {
      return new Problem(removed.errorType, { detail: removed.errorMsg }).send(
        res
      );
    }
  }
...
const findAndRender = async (hash, req, res) => {
  ...
  if (!template.success) {
    return new Problem(template.errorType, { detail: template.errorMsg }).send(
      res
    );
  } else {
...
router.get("/fileTypes", async (req, res) => {
  ...
  if (carboneRenderer.fileTypes instanceof Object) {
    ...
    return new Problem(500, {
      detail: "Unable to get file types dictionary",
    }).send(res);
  }
});
